...
class BaseTask(object):
    ...
    def get_num_subgoals(self, high_pddl):
        ...
        if last_action['planner_action']['action'] == 'End':
            return len(high_pddl) - 1  # ignore NoOp/End action (忽略 NoOp/End 动作)
        else:
    ...
    def transition_reward(self, state):
        ...
        # step and check if max_episode_length reached (步进并检查是否达到最大 episode 长度)
        self.step_num += 1
    ...
    def get_targets(self):
        ...
        # slice exception (切片异常)
        if 'object_sliced' in self.traj['pddl_params'] and self.traj['pddl_params']['object_sliced']:
            targets['object'] += 'Sliced'  # Change, e.g., "Apple" -> "AppleSliced" as pickup target. (更改，例如 "Apple" -> "AppleSliced" 作为拾取目标)
