...
class ThorEnv(Controller):
    ...
    ai2thor.controller.Controller 的扩展，用于 ALFRED 任务
    '''
    def __init__(self, x_display=constants.X_DISPLAY,
        ...
        # intemediate states for CoolObject Subgoal (CoolObject 子目标的中间状态)
    ...
    def reset(self, scene_name_or_num,
        ...
        # reset task if specified (如果指定了任务，则重置任务)
        if self.task is not None:
            self.task.reset()

        # clear object state changes (清除对象状态更改)
        self.reset_states()
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False):
        # TODO: parametrized navigation commands (参数化导航命令)
        ...
        if "RotateLeft" in action:
            ...
            # check if agent is holding knife in hand (检查代理是否手持刀具)
    ...
    def check_clean(self, object_id):
        ...
        处理 Faucet（水龙头）打开时的特殊情况。
        在这种情况下，我们需要对相应水槽盆中的每个对象执行 `CleanAction`。这是为了清洗水槽中的所有东西，而不仅仅是接触水流的东西。
        '''
        event = self.last_event
    ...
    def va_interact(self, action, interact_mask=None, smooth_nav=True, mask_px_sample=1, debug=False):
        ...
        if type(interact_mask) is str and interact_mask == "NULL":
            ...
            # ground-truth instance segmentation mask from THOR (来自 THOR 的地面实况实例分割掩码)
            instance_segs = np.array(self.last_event.instance_segmentation_frame)
            color_to_object_id = self.last_event.color_to_object_id

            # get object_id for each 1-pixel in the interact_mask (获取 interact_mask 中每个 1 像素的对象 ID)
            ...
            # iou scores for all instances (所有实例的 IoU 分数)
            iou_scores = {}
            ...
            # get the most common object ids ignoring the object-in-hand (获取最常见的对象 ID，忽略手中的对象)
            ...
            # prune invalid instances like floors, walls, etc. (修剪无效实例，如地板、墙壁等)
